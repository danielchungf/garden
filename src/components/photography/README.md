# Photography flythrough

A 3D "flying through a field of photos" visualizer at `/photography`. Photos drift
toward the viewer through perspective space; you can click any one to open it in the
shared lightbox.
...
| File | Role |
| --- | --- |
| `src/app/photography/page.tsx` | Server component. Reads `public/photography/` at **build time**, wraps the view in `LightboxProvider`, renders the back button + `PhotoFlythrough`. |
| `src/components/photography/PhotoFlythrough.tsx` | Client component. The whole effect — layout, animation loop, input, recycling. |
| `public/photography/*.webp` | The photos. |
| Home icon | `src/app/page.tsx` — the `Camera` `IconButton` linking to `/photography`. |

## Adding / changing photos

1. Drop image files into `public/photography/`. The page auto-discovers anything
   matching `jpe?g|png|webp|avif|gif` (see `getPhotos()` in `page.tsx`) and sorts by
   filename — **there is no list to maintain.**
